...
ros2 launch amzsim_interface amzsim_interface.launch.py \
	discipline_name_arg:=acceleration \
	track_path_arg:=/home/ubuntu/autonomous_2024/install/amzsim_tracks/share/amzsim_tracks/tracks_csv/standard/acceleration/Acceleration.csv \
	...
	show_fov_cones_arg:=True \
	show_ego_frame_arg:=True \
	show_veh_frame_arg:=True \
	show_online_map_arg:=True \
	show_bounded_path_arg:=True \
	show_delaunay_arg:=False \
	show_control_bounds_arg:=False \
	tv_ff_arg:=1.0 \
	tv_exp_arg:=2.45 \
	tv_p_arg:=325.0 \
	tv_i_arg:=0.0 \
