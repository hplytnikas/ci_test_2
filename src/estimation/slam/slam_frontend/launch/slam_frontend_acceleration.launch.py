...
def generate_launch_description():
    ...
    # Discipline specific parameters, make sure they are after the default parameters in LaunchDescription
    config_acceleration = os.path.join(
        get_package_share_directory("slam_frontend"),
        "config",
        "params_acceleration.yaml",
    )
    ...
    return LaunchDescription(
        [
            Node(
                ...
                name=node_name,
                output="screen",
                parameters=[config_slam, config_visualizer, config_acceleration],
            ),
        ]
    )
